...
SRC = arch/cpu/mmu.c arch/cpu/entry.S kernel/threads.c kernel/scheduler.c arch/cpu/stacks.c kernel/start.c kernel/kprintf.c kernel/panic.c arch/bsp/yellow_led.c lib/ubsan.c lib/mem.c arch/bsp/uart.c lib/fmt.c lib/kvprintf.c kernel/exceptions.c arch/bsp/irq.c arch/bsp/systimer.c kernel/exception_print.c arch/cpu/vectors.S arch/cpu/traps.S arch/cpu/init_stacks.S arch/cpu/cpu.c tests/regcheck_asm.S tests/regcheck.c arch/cpu/banked_regs.c kernel/exc_frame_layout.c

# Hier separate user source files hinzufügen
USRC = user/06_main.c user/syscalls.S user/uprintf.c
...
SUBMISSION_BRANCH = a6-address-spaces-multithreading
...
ifneq ($(BIN_LSG), )
	BINLSG_LDFLAGS = -L$(BIN_LSG_DIR) $(addprefix -l,$(BIN_LSG))
else
	BINLSG_LDFLAGS =
endif
...
ifneq ($(words $(OBJ)), $(words $(SRC) $(USRC)))
	...
	$(LD) -o $@ $(wordlist 2,$(words $^),x $^) $(LDFLAGS) $(BINLSG_LDFLAGS)
endif
